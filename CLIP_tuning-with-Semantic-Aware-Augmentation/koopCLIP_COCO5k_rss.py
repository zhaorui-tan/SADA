...
if __name__ == '__main__':
    ...
    save_dir = f'./outputs_clip_COCO5k_ce_ub_cro_cov_bs_nz_ema_onlyT{str(args.batch_size)}_{str(args.lr)}_eigen_w{str(eigen_w)}_cov_w{str(cov_w)}_cross_w{str(cross_w)}_rss_w{str(rss_w)}'
    ...
    for epoch in range(args.epoch):
        ...
        for data in pbar:
            ...
            eigen_image = torch.linalg.svdvals(image_features.float())
            eigen_text = torch.linalg.svdvals(text_features.float())

            eigen_image_loss =(nn.MSELoss()(eigen_image, torch.mean(eigen_image)) +
                               torch.sum(torch.relu(-(eigen_image - t))))
            eigen_text_loss = (nn.MSELoss()(eigen_text, torch.mean(eigen_text)) +
                               torch.sum(torch.relu(-(eigen_text - t))))
            cross_loss = nn.MSELoss()(torch.mean(eigen_image), torch.mean(eigen_text)) * 0

            # eigen_image_loss = (criterion(eigen_image.half(), torch.mean(eigen_image)) +
            #                     torch.sum(torch.relu(-(eigen_image - t))))
            # eigen_text_loss = (criterion(eigen_text, torch.mean(eigen_text)) +
            #                    torch.sum(torch.relu(-(eigen_text - t))))
            # cross_loss = criterion(torch.mean(eigen_image), torch.mean(eigen_text)) * 0
            # eigen_image_loss = nn.MSELoss()(torch.max(eigen_image).detach(), torch.min(eigen_image)) + 100 * torch.sum(torch.relu(-eigen_text))
            # eigen_text_loss = nn.MSELoss()(torch.max(eigen_text).detach(), torch.min(eigen_text)) + 100 * torch.sum(torch.relu(-eigen_image))

            rss_i2t = get_rss(image_features, text_features)
            ...
            total_loss = loss + cov_loss * cov_w + cross_loss * cross_w +\
                         (eigen_image_loss + eigen_text_loss ) * eigen_w / 2 + \
                         (rss_i2t + rss_t2i) * rss_w / 2

            pbar.set_description(f'loss_img_txt: {loss.item():.4f}, '
                                 f'cov_loss: {cov_loss.item() * cov_w:.4f}, '
                                 f'cross_loss: {cross_loss.item():.4f}, '
            ...
            writer.add_scalar(f'train/cov_loss', cov_loss.item(), step)
            writer.add_scalar(f'train/cross_loss', cross_loss.item(), step)
            writer.add_scalar(f'train/eigen_image_loss', eigen_image_loss.item(), step)
            writer.add_scalar(f'train/eigen_text_loss', eigen_text_loss.item(), step)
