...
if __name__ == '__main__':
    ...
    N = 128 * 10
    save_dir = f'./outputs_clip_COCO5k_tune_SADA_N{N}_ITA_{str(args.lr)}'
    ...
    for epoch in range(args.epoch):
        ...
        for data in pbar:
            ...
            total_loss = loss # + (rss_i2t + rss_t2i)/2 +\(eigen_image_loss + eigen_text_loss)/2

            # n1 = ((torch.randint(0, 2, (args.batch_size, 512)) - 0.5) * 2.).to('cuda')
            n1 = torch.randn((args.batch_size, 512))- 0.5
            n1 = n1.to('cuda')
            text_features_ = aug(n1, text_features)

            text_features_ = text_features_.half()
            logit_scale = model.logit_scale.exp()
            logits_per_image = logit_scale * image_features @ text_features_.t()
            logits_per_text = logits_per_image.t()
            ground_truth = torch.arange(len(imgs), dtype=torch.long, device=device)
            loss1_ = criterion(logits_per_image, ground_truth)
            loss2_ = criterion(logits_per_text, ground_truth)
            loss_ = (loss1_ + loss2_) / 2.
            total_loss += loss_

            total_loss/=2.

            mse = nn.MSELoss()(text_features_, text_features)
