...
def get_one_batch_data(dataloader, text_encoder, ixtoword, args):
    print('++++++++++++++++++++++++++++++')
...
def get_fix_data(train_dl, test_dl, text_encoder, ixtoword, args):
    fixed_image_train, fixed_word_train, fixed_sent_train = get_one_batch_data(train_dl, text_encoder, ixtoword, args)
    fixed_image_test, fixed_word_test, fixed_sent_test = get_one_batch_data(test_dl, text_encoder, ixtoword, args)
...
def prepare_data(data, ixtoword):
    ...
    for b in range(len(captions)):
        if is_tensor:
            tmp_w = [ixtoword[k[0]] for k in captions[b].to('cpu').numpy().tolist() if k[0] != 0]
        else:
            tmp_w = [ixtoword[k[0]] for k in captions[b] if k[0] != 0]
        tmp_s = ' '.join(tmp_w)
        words.append(tmp_w)
        sent.append(tmp_s)
    return imgs, sent

def drop_tokens(embeddings, max_len, word_dropout_r=0.2):
...
def prepare_data_random_mask(data, text_encoder, ixtoword, args, return_captions=False):
    if args.TEXT.USE_CLIP:
        imgs, captions, caption_lens, keys = data
        imgs = Variable(imgs).cuda()
        words = []
        ...
        for b in range(len(captions)):
            ...
            words.append(tmp_w)
            sent.append(tmp_s)
        _, sent_emb = clip_text_embedding(text=sent, clip_model=text_encoder)
        words_embs = None
    else:
        imgs, captions, caption_lens, keys = data
        imgs = Variable(imgs).cuda()
    ...
    if return_captions:
        return imgs, sent_emb, words_embs, keys, captions
    else:
...
def clip_text_embedding(text, clip_model):
    text = clip.tokenize(text).cuda()
    text_features = clip_model.encode_text(text)
    return text, text_features.float()

def clip_image_embedding(image, clip_model):
    image = image.cuda()
    image = clip_trans(clip_pool(image))
...
def get_imgs(img_path, bbox=None, transform=None, normalize=None, preprocess=None):
...
class TextImgDataset(data.Dataset):
    def __init__(self, split='train', transform=None, args=None, preprocess=None):
    ...
    def load_filenames(self, data_dir, split):
        ...
        print(f'loading file names from {filepath} ')
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_name.find('coco') != -1:
            if self.split == 'train':
                ...
                img_name = '%s/images/train/train2014/train2014/%s.jpg' % (data_dir, key)
            else:
                # img_name = '%s/images/val2014/%s.jpg' % (data_dir, key)
                img_name = '%s/images/val/val2014/val2014/%s.jpg' % (data_dir, key)
        ...
        imgs = get_imgs(img_name, bbox, self.transform, normalize=self.norm, preprocess=self.preprocess)
