...
if __name__ == '__main__':
    ...
    save_dir = './outputs_clip_rss_double_1e-5'
    ...
    for epoch in range(args.epoch):
        ...
        for data in pbar:
            ...
            image_cov = torch.cov(image_features).float() * 0
            mask = 1 - torch.eye(len(image_cov)).float().cuda()
            image_cov_loss = nn.MSELoss()(image_cov * mask, torch.zeros_like(image_cov))

            text_cov = torch.cov(text_features).float().cuda() * 0
            text_cov_loss = nn.MSELoss()(text_cov * mask, torch.zeros_like(text_cov))
            ...
            total_loss = ( loss1+loss2 ) / 2 + image_cov_loss + text_cov_loss + (rss_i2t + rss_t2i)/2
