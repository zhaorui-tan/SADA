...
if __name__ == '__main__':
    ...
    save_dir = f'./outputs_clip_rss_double_eigen_rss_cov2_COCO5k_{str(args.lr)}_eigen_w{str(eigen_w)}_cov_w{str(cov_w1)}_{str(cov_w2)}'
    ...
    for epoch in range(args.epoch):
        ...
        #         # idx = idx.unsqueeze(1).repeat(1,5)
        #         # idx = idx.view(-1)
        #         # print(idx)
        ...
        for data in pbar:
            ...
            eigen_image_loss =(nn.MSELoss()(eigen_image, torch.mean(eigen_image)) +
                               torch.sum(torch.relu(-(eigen_image - t)))) * eigen_w
            eigen_text_loss = (nn.MSELoss()(eigen_text, torch.mean(eigen_text)) +
                               torch.sum(torch.relu(-(eigen_text - t)))) * eigen_w
            cross_loss = nn.MSELoss()(torch.mean(eigen_image), torch.mean(eigen_text)) * 100
            ...
            # rss_i2t = get_rss(image_features, text_features) *  rss_w
            # rss_t2i = get_rss(text_features, image_features) *  rss_w
            ...
            pbar.set_description(f'loss_img_txt: {loss.item():.4f}, '
                                 ...
                                 f'image_cov_loss: {image_cov_loss.item():.4f}, '
                                 f'text_cov_loss: {text_cov_loss.item():.4f}, '
            ...
            writer.add_scalar(f'train/image_cov_loss', image_cov_loss.item(), step)
            writer.add_scalar(f'train/text_cov_loss', text_cov_loss.item(), step)
