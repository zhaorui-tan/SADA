...
def load_model_opt(netG, netD, netC, aug, optim_G, optim_D, optim_S, path, multi_gpus):
    checkpoint = torch.load(path, map_location=torch.device('cpu'))
    netG = load_model_weights(netG, checkpoint['model']['netG'], multi_gpus)
    netD = load_model_weights(netD, checkpoint['model']['netD'], multi_gpus)
    netC = load_model_weights(netC, checkpoint['model']['netC'], multi_gpus)
    ...
    optim_D = load_opt_weights(optim_D, checkpoint['optimizers']['optimizer_D'])
    try:
        aug = load_model_weights(aug, checkpoint['model']['aug'], multi_gpus)
        optim_S = load_opt_weights(optim_S, checkpoint['optimizers']['optimizer_S'])
    except Exception:
        print('unable to load aug and optim_S')

    print('loaded models and opts')
    return netG, netD, netC, aug, optim_G, optim_D, optim_S
...
def load_models(netG, netD, netC, aug, path, multi_gpus):
    checkpoint = torch.load(path, map_location=torch.device('cpu'))
    netG = load_model_weights(netG, checkpoint['model']['netG'], multi_gpus)
    netD = load_model_weights(netD, checkpoint['model']['netD'], multi_gpus)
    netC = load_model_weights(netC, checkpoint['model']['netC'], multi_gpus)
    try:
        ...
        print('unable to load aug')
    return netG, netD, netC, aug
...
def load_netG(netG, aug, path, multi_gpus, train):
    checkpoint = torch.load(path, map_location="cpu")
    netG = load_model_weights(netG, checkpoint['model']['netG'], multi_gpus, train)
    if 'aug' in checkpoint['model']:
        try:
            aug = load_model_weights(aug, checkpoint['model']['aug'], multi_gpus, train)
        except Exception:
            print('unable to load aug')
            pass
    return netG, aug
...
def save_models(netG, netD, netC, aug, optG, optD, optS, epoch, multi_gpus, save_path):
    if (multi_gpus == True) and (get_rank() != 0):
        None
    else:
        state = {'model': {'netG': netG.state_dict(), 'netD': netD.state_dict(), 'netC': netC.state_dict(), 'aug': aug.state_dict()},
                 'optimizers': {'optimizer_G': optG.state_dict(), 'optimizer_D': optD.state_dict(), 'optimizer_S': optS.state_dict() },
                 'epoch': epoch}
        torch.save(state, '%s/state_epoch_%03d.pth' % (save_path, epoch))
