...
def sample_example(wordtoix, netG, aug, text_encoder, args):
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(caption_num)):
            ...
            vutils.save_image(fake_aug.data, img_name, nrow=4, range=(-1, 1), normalize=True)
...
def main(args):
    ...
    netG, aug = load_netG(netG, aug, model_path, args.multi_gpus, train=False)
    netG.eval()
    aug.eval()
    ...
    start_t = time.time()
    sample_example(wordtoix, netG, aug, text_encoder, args)
    end_t = time.time()
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
