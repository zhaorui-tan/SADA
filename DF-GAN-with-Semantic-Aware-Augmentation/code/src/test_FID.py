...
from lib.modules_for_review import eval
...
#     netG, _ = load_netG(netG, aug, model_path, multi_gpus, train=False)
#     netG.eval()
...
#                         args.sample_times, args.z_dim, args.batch_size, args.truncation, args.trunc_rate, args=args)
#     end_t = time.time()
#     if (multi_gpus==True) and (get_rank() != 0):
#         None
#     else:
...
def main(args):
    ...
    model_path = osp.join(ROOT_PATH, checkpoint)
    netG, _ = load_netG(netG, aug, model_path, multi_gpus, train=False)
    netG.eval()
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    ...
    if (multi_gpus==True) and (get_rank() != 0):
        ...
        print('Sampling done, %.2fs cost, The FID is : %.2f'%(end_t-start_t, fid))
        print(f'current epoch: {checkpoint}')
