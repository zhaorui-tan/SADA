...
from lib.modules_for_review import eval_cs
...
def main(args):
    ...
    args.val_save_dir = osp.join(args.checkpoint[:-4], time_stamp)
    ...
    image_encoder, text_encoder, netG, netD, netC, aug = prepare_models(args)
    ...
    netG, aug = load_netG(netG, aug, model_path, multi_gpus, train=False)
    netG.eval()
    if (args.multi_gpus == True) and (get_rank() != 0):
        ...
        print("************ Start testing CS ************")
    start_t = time.time()
    m1, s1 = load_npz(args.npz_path)
    with torch.no_grad():
        ssd_result = eval_cs(valid_dl, text_encoder, netG, args.device, m1, s1, args.save_image, args.val_save_dir,
                              args.sample_times, args.z_dim, args.batch_size, args.truncation, args.trunc_rate,
                              ixtoword=ixtoword, args=args)
        # from lib.modules import eval_ssd_damsm
        # ssd_result = eval_ssd_damsm(valid_dl, text_encoder,image_encoder, netG, args.device, m1, s1, args.save_image, args.val_save_dir,
        #                       args.sample_times, args.z_dim, args.batch_size, args.truncation, args.trunc_rate,
        #                       ixtoword=ixtoword, args=args)
    end_t = time.time()
    if (multi_gpus == True) and (get_rank() != 0):
