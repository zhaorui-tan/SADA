...
from model_cat_noise_D import G_DCGAN, G_NET
from datasets import prepare_data
from model_cat_noise_D import RNN_ENCODER, CNN_ENCODER
    ...
    def build_models(self):
        ...
        if cfg.GAN.B_DCGAN:
            if cfg.TREE.BRANCH_NUM == 1:
                from model_cat_noise_D import D_NET64 as D_NET
            elif cfg.TREE.BRANCH_NUM == 2:
                from model_cat_noise_D import D_NET128 as D_NET
            else:  # cfg.TREE.BRANCH_NUM == 3:
                from model_cat_noise_D import D_NET256 as D_NET
            # TODO: elif cfg.TREE.BRANCH_NUM > 3:
            netG = G_DCGAN()
            netsD = [D_NET(b_jcu=False)]
        else:
            from model_cat_noise_D import D_NET64, D_NET128, D_NET256
            netG = G_NET()
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                errD_total = 0
                D_logs = ''
                c_z_code = torch.cat((noise, sent_emb), 1)
                for i in range(len(netsD)):
                    netsD[i].zero_grad()
                    errD = discriminator_loss(netsD[i], imgs[i], fake_imgs[i],
                                              c_z_code, real_labels, fake_labels)
                ...
                errG, G_logs = \
                    generator_loss(netsD, image_encoder, fake_imgs, real_labels,
                                   words_embs, sent_emb, c_z_code, match_labels, cap_lens, class_ids)
