...
from model_train_aug import G_DCGAN, G_NET
from datasets import prepare_data
from model_train_aug import RNN_ENCODER, CNN_ENCODER


from miscc.losses import words_loss
from miscc.losses import discriminator_loss, generator_loss, KL_loss, aug_loss, augG_loss
import os
import time
import numpy as np
import sys
...
# from ssd_tf import draw_distributions
...
class condGANTrainer(object):
    ...
    def build_models(self):
        ...
        if cfg.TRAIN.NET_G != '':
            ...
            missing_k_list = [ k for k in current_dict if k not in state_dict]
            print(missing_k_list)
            print('*' * 10)
    ...
    def train(self):
        ...
        # print(f'using aug {p1, p2}')
        # if cfg.DATASET_NAME == 'birds':
        #     aug = SemanticAug('/data1/phd21_zhaorui_tan/PDF_GAN_ssdloss/cov/DAMSM/birds/', 256, p1=p1, p2=p2)
        # else:
        #     aug = SemanticAug('/data1/phd21_zhaorui_tan/PDF_GAN_ssdloss/cov/DAMSM/coco/', 256, p1=p1, p2=p2)
        ...
        try:
            ...
            # netG.load_state_dict(state_dict)
            current_dict = aug.state_dict()
            pretrained_dict = {k: v for k, v in state_dict.items() if k in current_dict}
            # 2. overwrite entries in the existing state dict
            # current_dict.update(pretrained_dict)
            # 3. load the new state dict
            aug.load_state_dict(pretrained_dict)
            # netG.load_state_dict(state_dict)
            print('Load aug from: ', aug_pth)
        except Exception:
            print('cannot load aug')
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                # if epoch >= warm_up:
                # n1 = torch.rand(batch_size, 256).to('cuda')
                # n2 = torch.rand(batch_size, 256).to('cuda')
                # n1 = ((torch.rand(batch_size, 256) - 0.5) * 2).to('cuda')
                # n2 = ((torch.rand(batch_size, 256) - 0.5) * 2).to('cuda')
                # sent_emb_aug, sent_emb_aug2 = aug(n1, n2)
                # sent_emb_aug += sent_emb.float() + sent_emb_aug2
                # sent_emb = sent_emb_aug
                # sent_emb_aug, sent_emb_aug2 = n1 * p1, n2 * p2
                # sent_emb_aug += sent_emb.float() + sent_emb_aug2
                # sent_emb = sent_emb_aug

                # fake_imgs, _, mu, logvar = netG(noise, sent_emb, words_embs, mask)
                fake_imgs, _, mu, logvar, = netG(noise, sent_emb, words_embs, mask)
                sent_emb_aug = aug(sent_emb, sent_emb).detach()
                delta_aug = sent_emb_aug - sent_emb
                words_embs_aug = words_embs + delta_aug.repeat(words_embs.shape[-1],1,1).permute(1,2,0)
                fake_imgs_aug, _, mu_aug, logvar_aug, = netG(noise, sent_emb_aug, words_embs_aug, mask)
                ...
                for i in range(len(netsD)):
                    ...
                    errD_aug = discriminator_loss(netsD[i], imgs[i], fake_imgs_aug[i],
                                              sent_emb_aug, real_labels, fake_labels)
                    # backward and update parameters
                    errD_all = errD + errD_aug
                    errD_all.backward()
                    optimizersD[i].step()
                    errD_total += errD + errD_aug
                    D_logs += 'errD%d: %.2f ' % (i, errD.item())
                    D_logs += 'errD_aug%d: %.2f ' % (i, errD_aug.item())
                ...
                aug_errG_loss, aug_G_logs = \
                    generator_loss(netsD, image_encoder, fake_imgs_aug, real_labels,
                                   words_embs_aug, sent_emb_aug, match_labels, cap_lens, class_ids)
                kl_loss_aug = KL_loss(mu_aug, logvar_aug)

                aug_dir_loss = aug_loss(image_encoder, fake_imgs, fake_imgs_aug, sent_emb, sent_emb_aug, real_labels,
                                        class_ids,
                                        batch_size)  # * 10

                # errG_total = errG + kl_loss + aug_errG_loss + kl_loss_aug + aug_dir_loss
                errG_total = errG + kl_loss + aug_errG_loss + kl_loss_aug
                ...
                # (4) Update Aug
                ######################################################
                sent_emb_aug = aug(sent_emb, sent_emb)
                for name, parameter in netG.named_parameters():
                    parameter.requires_grad = False
                fake_imgs_aug, _, mu, logvar, = netG(noise, sent_emb_aug, words_embs_aug, mask)
                semantic_bounding, aug_G_logs = \
                    augG_loss(netsD, image_encoder, fake_imgs_aug, real_labels,
                                   words_embs, sent_emb, match_labels, cap_lens, class_ids)
                mse = - mse_loss(sent_emb, sent_emb_aug)
                # aug_mse = torch.exp(mse)
                aug_mse = torch.exp(- mse_loss(sent_emb, sent_emb_aug)) - 0.2
                aug_mse = aug_mse if aug_mse > 0 else aug_mse * 0
                a = 0
                aug_total = semantic_bounding * (1 - a) + aug_mse * a
                ...
                #         avg_p.mul_(0.999).add_(0.001, p.data)
                # else:
                #     G_logs += 'fixed kl_loss: %.2f ' % kl_loss.item()

                if gen_iterations % 50 == 0:
                    print(D_logs + G_logs)
                    # print(
                    # f'aug: aug_mse {round(aug_mse, 3)},  aug_dir_loss {round(aug_dir_loss, 3)}, aug_errG_loss {round(aug_errG_loss, 3)}')
                    print(
                        f'aug: aug_mse: %.2f  mse: %.2f  aug_dir_loss: %.2f, aug_errG_loss: %.2f, semantic_bounding: %.2f, aug_total: %.2f' % (
                            aug_mse.item(),  mse.item(), aug_dir_loss.item(), aug_errG_loss.item(), semantic_bounding.item(), aug_total.item()))
                # save images
                ...
                #     fake_imgs_for_save = fake_imgs[-1]
                ...
                if gen_iterations % 500 == 0:
                    ...
                    aug_fake_imgs_for_save = fake_imgs_aug[-1].detach().cpu()
                    fullpath = '%s/G_%d_%d_aug.png' \
                               % (self.image_dir, gen_iterations, epoch)
                    vutils.save_image(aug_fake_imgs_for_save.data,
                                      fullpath,
                                      normalize=True)
                    load_params(netG, backup_para)
            end_t = time.time()
            ...
            if epoch % cfg.TRAIN.SNAPSHOT_INTERVAL == 0:  # and epoch != 0:
                ...
                torch.save(aug.state_dict(),
                           '%s/aug_epoch_%d.pth' % (self.model_dir, epoch))
    ...
    def sampling(self, split_dir, use_aug=True, save_image =False):
        if cfg.TRAIN.NET_G == '':
    ...
    def sampling_draw(self, split_dir):
        import tensorflow as tf
        from cfid import cfid
        os.environ['MKL_NUM_THREADS'] = '1'
        gpu = tf.config.list_physical_devices('GPU')
            ...
            for i in range(cfg.TEXT.CAPTIONS_PER_IMAGE + 1):  # (cfg.TEXT.CAPTIONS_PER_IMAGE):
                for step, data in enumerate(self.data_loader, 0):
                    ...
                    for j in range(batch_size):
                        ...
                        if R_count >= n:
                            draw_distributions(save_dir, all_real, all_fake, all_caps, )
                            print('data loaded')
    ...
    def gen_example(self, data_dic):
        if cfg.TRAIN.NET_G == '':
            ...
            # print('*' * 100)
            # missing_k_list = [k for k in state_dict if k not in current_dict]
            # print(missing_k_list)
            # missing_k_list = [k for k in current_dict if k not in state_dict]
            # print(missing_k_list)
            # print('*' * 100)
            ...
            for key in data_dic:
                ...
                for i in range(20):  # 16
                    ...
                    for j in range(batch_size):
                        save_name = '%s/%d_s_%d' % (save_dir,sorted_indices[j], i,)
                        # for k in range(len(fake_imgs[-1])):
                        ...
                        # print('im', im.shape)
                        im = Image.fromarray(im)
                        # fullpath = '%s_g%d.png' % (save_name, i)
                        fullpath = '%s_g.png' % (save_name)
                        im.save(fullpath)
