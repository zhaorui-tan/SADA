...
def symmetric_matrix_square_root(mat, eps=1e-10):
    ...
    # return torch.matmul(torch.matmul(u, torch.linalg.diagonal(si)), v, transpose_b=True)
...
def cfid(y_true, y_predict, x_true, embeding=no_embedding, estimator=sample_covariance):
    ...
    c_dist1 = tf.linalg.trace(tf.matmul(tf.matmul(c_y_true_x_true_minus_c_y_predict_x_true, inv_c_x_true_x_true),
                                        c_x_true_y_true_minus_c_x_true_y_predict))
    # c_dist2 = tf.linalg.trace(
    #     c_y_true_given_x_true + c_y_predict_given_x_true) - 2 * trace_sqrt_product(
    #     c_y_predict_given_x_true, c_y_true_given_x_true)

    c_dist3_m = tf.einsum('...k,...k->...', m_x_true - m_y_predict, m_x_true - m_y_predict)
    c_dist3 = tf.linalg.trace(c_x_true_x_true + c_y_predict_y_predict) - 2 * trace_sqrt_product(
        c_y_predict_y_predict, c_x_true_x_true)

    # c_dist3_c = tf.norm(tf.linalg.diag_part(c_y_predict_given_x_true - c_y_true_given_x_true))
    c_dist3_c = tf.math.sqrt(tf.math.square(tf.linalg.diag_part(c_y_predict_given_x_true - c_y_true_given_x_true)))
    c_dist3_c = tf.reduce_sum(c_dist3_c)
    # print('c_dist3_m')
    # tf.print(c_dist3_m)
    # print('c_dist3_c', )
    # tf.print(c_dist3_c)

    # c_dist3_r_m = tf.einsum('...k,...k->...', m_x_true - m_y_true, m_x_true - m_y_true)
    # c_dist_r = tf.linalg.trace(c_x_true_x_true + c_y_true_y_true) - 2 * trace_sqrt_product(
    #     c_y_true_y_true, c_x_true_x_true)
    # c_dist_r = c_dist_r + c_dist3_r_m
    # print('c_dist_r', )
    # tf.print(c_dist_r)
    # c_dist3_r_m = tf.einsum('...k,...k->...', m_x_true - m_y_true, m_x_true - m_y_true)
    # c_dist_r = tf.linalg.trace(c_x_true_x_true + c_y_true_y_true) - 2 * trace_sqrt_product(
    ...
    # c_dist_r = tf.linalg.trace((c_x_true_x_true * mask) + (c_y_true_y_true * mask)) - 2 * trace_sqrt_product(
    #     (c_y_true_y_true * mask), (c_x_true_x_true * mask))
    # # return c_dist3_m + c_dist3, c_dist3_c
    mask = tf.cast(tf.linalg.diag(tf.ones([512])), dtype=tf.float64)
    c_dist_r = tf.linalg.trace(c_y_true_given_x_true * mask + c_y_predict_given_x_true * mask) - 2 * trace_sqrt_product(
        c_y_predict_given_x_true * mask, c_y_true_given_x_true * mask)
    print('c_dist_r', )
    tf.print(c_dist_r)

    return c_dist3_c, c_dist_r
