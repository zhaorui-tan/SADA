    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                n1 = ((torch.rand(batch_size, 256) - 0.5) * 2).cuda()
                sent_emb_aug = aug(n1, sent_emb).detach()
                ...
                with torch.no_grad():
                    _, eff = image_encoder(fake_imgs_aug[-1])
                    _, ef = image_encoder(fake_imgs[-1])
                r = R(eff, ef, n1)

                errG_total = errG + kl_loss + aug_errG_loss + kl_loss_aug + r_p * r.double()
                ...
                if gen_iterations % 50 == 0:
                    ...
                    print(
                        f'aug: r: %.2f, aug_errG_loss: %.2f' % (
                            r.item(), aug_errG_loss.item(), )
                    )
