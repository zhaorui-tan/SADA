...
def mixup_data(x, y, yw, alpha=1.0, use_cuda=True):
    ...
    y_a, y_b = y, y[index]
    yw_a, yw_b = yw, yw[index]
    return mixed_x, y_a, y_b, yw_a, yw_b, lam
...
class condGANTrainer(object):
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                ######################################################
                # data = data_iter.next()
                imgs_, captions, cap_lens, class_ids, keys = prepare_data(data)
                ...
                # imgs, targets_a, targets_b, targets_w_a, targets_w_b, lam = mixup_data(imgs, sent_emb, words_embs)
                # sent_emb = lam * targets_a + (1 - lam) * targets_b
                # words_embs = lam * targets_w_a + (1 - lam) * targets_w_b
                ...
                fake_imgs__, _, mu, logvar, = netG(noise, sent_emb, words_embs, mask)

                imgs = [torch.ones_like(imgs_[i]).cuda() for i in range(len(imgs_))]
                fake_imgs = [torch.ones_like(fake_imgs__[i]).cuda() for i in range(len(fake_imgs__))]
                for i in range(len(imgs_)):
                    both_monet = torch.concat([imgs_[i], fake_imgs__[i]], dim=0)
                    aug_monet = aug_fn(both_monet)
                    imgs[i] = aug_monet[:batch_size]
                    fake_imgs[i] = aug_monet[batch_size:]
                ...
                for i in range(len(netsD)):
                    ...
                    errD_all = errD
                    errD_all.backward(retain_graph=True)
                    optimizersD[i].step()
                    errD_total += errD
    ...
    def sampling_ssd(self, split_dir):
        ...
        if cfg.TRAIN.NET_G == '':
            ...
            #
            # paths = ['/data1/phd21_zhaorui_tan/SADA/AttnGAN/output/birds_attn2_2023_03_16_17_57_32/Model/netG_epoch_600',
            #          '/data1/phd21_zhaorui_tan/data_raw/birds/CUB_200_2011/images']
            # fid_value = calculate_fid_given_paths(paths, batch_size, gpu[0], 2048)
            # result = f'FID: {fid_value}'
            # print(result)
