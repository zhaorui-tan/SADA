...
from miscc.losses import discriminator_loss, generator_loss, KL_loss, aug_loss
import os
import time
import numpy as np
import sys
...
class condGANTrainer(object):
    ...
    def train(self):
        ...
        if cfg.DATASET_NAME == 'birds':
            p = 0.05
            print('using birds')
            aug = SemanticAug('/data1/phd21_zhaorui_tan/data_raw/cov/DAMSM/birds/', 256, p=p).cuda()
        else:
            p = 0.01
            print('using coco')
            aug = SemanticAug('/data1/phd21_zhaorui_tan/data_raw/cov/DAMSM/coco/', 256, p=p).cuda()
        print(f'using aug {p}')
        warm_up = 0
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                fake_imgs, _, mu, logvar, = netG(noise, sent_emb, words_embs, mask)
                # n1 = ((torch.rand(batch_size, 256) - 0.5) * 2).to('cuda') * min(1, epoch / (self.max_epoch / 2))
                n1 = ((torch.rand(batch_size, 256) - 0.5) * 2).to('cuda')
                sent_emb_aug = aug(n1, sent_emb).cuda()
                ...
                errG_total = errG + kl_loss + aug_errG_loss + kl_loss_aug + aug_dir_loss * 1
                ...
                if gen_iterations % 50 == 0:
                    ...
                    print(
                        f'aug: aug_dir_loss: %.2f, aug_errG_loss: %.2f' % (
                            aug_dir_loss.item(), aug_errG_loss.item(), )
                    )
